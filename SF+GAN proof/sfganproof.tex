...
\title{proof: SF+GAN Minimizes Expected Jensen--Shannon Divergence}
\author{}
\date{}
...
The SF+GAN training procedure is summarized in Algorithm~\ref{alg:sfgan}. We update the discriminator by ascending:
\begin{align}
\nabla_{\theta_D} \frac{1}{m} \sum_{i=1}^{m}
\Big[ 
...
\begin{algorithm}[t]
\caption{SF+GAN Training for Videos}
\label{alg:sfgan}
\KwIn{Iterations $N$, batch size $m$, $p_z$, $p_{\text{data}}$, $p(k)$, forward process $F$, generator $G$, discriminator $D$}
\KwOut{Trained generator $G$}
Initialize $G$ and $D$ randomly\;
\For{$n = 1, \ldots, N$}{
    Sample $\{z_{0:T}^{(i)}\}_{i=1}^{m} \sim p_z$, $\{x_{0:T}^{(i)}\}_{i=1}^{m} \sim p_{\text{data}}$\;
    Sample noise schedules: $\{k_{0:T}^{(i), \text{real}}\}_{i=1}^{m}, \{k_{0:T}^{(i), \text{fake}}\}_{i=1}^{m} \sim p(k)$\;
    Generate $\tilde{x}_{0:T}^{(i)} = G(z_{0:T}^{(i)})$, apply forward process:
    $x_{0:T}^{(i), \text{noisy}} = F(x_{0:T}^{(i)}, k_{0:T}^{(i), \text{real}})$, 
    $\tilde{x}_{0:T}^{(i), \text{noisy}} = F(\tilde{x}_{0:T}^{(i)}, k_{0:T}^{(i), \text{fake}})$\;
    Update $D$: $\nabla_{\theta_D} \frac{1}{m} \sum_{i=1}^{m} \big[ \log D(x_{0:T}^{(i), \text{noisy}}; k_{0:T}^{(i), \text{real}}) + \log (1 - D(\tilde{x}_{0:T}^{(i), \text{noisy}}; k_{0:T}^{(i), \text{fake}})) \big]$\;
...
p_{\text{data}}(x_{0:T}^{\text{noisy}} | k_{0:T}^{\text{real}}) 
\log D(x_{0:T}^{\text{noisy}}; k_{0:T}^{\text{real}}) \\
&\quad + p_g(x_{0:T}^{\text{noisy}} | k_{0:T}^{\text{fake}}) 
\log \big(1 - D(x_{0:T}^{\text{noisy}}; k_{0:T}^{\text{fake}})\big)
\Big] 
\, dx_{0:T}^{\text{noisy}}.
\end{aligned}
