...
\label{alg:sfgan}
\KwIn{Number of training iterations $N$, batch size $m$, noise distribution $p_z$, data distribution $p_{\text{data}}$, noise schedule distribution $p(k)$, forward process $F$, generator $G$, discriminator $D$}
\KwOut{Trained generator $G$}
Initialize generator $G$ and discriminator $D$ with random weights\;
\For{training iteration $n = 1, 2, \ldots, N$}{
